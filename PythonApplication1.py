...
from arcs import gArc_test as arctest
Start=True
window = tk.Tk()
...
canvas_arc1=canvas.create_arc(arc1.xy, arc1.xy[0]+arc1.size[0], arc1.xy[1]+arc1.size[1], start=arc1.startC, extent=arc1.extentC, fill=arc1.color)
canvas_arc2=canvas.create_arc(arc2.xy, arc2.xy[0]+arc2.size[0], arc2.xy[1]+arc2.size[1], start=arc2.startC, extent=arc2.extentC, fill=arc2.color)
canvas_arc3=canvas.create_arc(arc3.xy, arc3.xy[0]+arc3.size[0], arc3.xy[1]+arc3.size[1], start=arc3.startC, extent=arc3.extentC, fill=arc3.color)

init_arcs(arctest,"a1",20,20,20,20,120,300,"red",5)
canvas_arctest=arctest

canvas_arctest.creat(canvas)
...
def arc1_Rotate_right():
    ...
    global arc1_xy
    arc1_startC -= 2
    canvas.delete(canvas_arc1)
    canvas_arc1=canvas.create_arc(arc1_xy,start=arc1_startC,extent=300,fill='red')
...
def arc2_Rotate_right():
    ...
    global arc2_xy
    arc2_start -= 2
    canvas.delete(canvas_arc2)
    canvas_arc2=canvas.create_arc(arc2_xy,start=arc2_start,extent=300,fill='blue')
def arc_Rotate_LR(who,LR):
    ...
    if who.index==1:
        global canvas_arc1
        canvas.delete(canvas_arc1)
        canvas_arc1=canvas.create_arc(who.xy, who.xy[0]+who.size[0], who.xy[1]+who.size[1],start=who.startC,extent=who.extentC,fill=who.color)
    elif who.index==2:
        global canvas_arc2
        canvas.delete(canvas_arc2)
        canvas_arc2=canvas.create_arc(who.xy, who.xy[0]+who.size[0], who.xy[1]+who.size[1],start=who.startC,extent=who.extentC,fill=who.color)
    elif who.index==3:
        global canvas_arc3
        canvas.delete(canvas_arc3)
        canvas_arc3=canvas.create_arc(who.xy, who.xy[0]+who.size[0], who.xy[1]+who.size[1],start=who.startC,extent=who.extentC,fill=who.color)
    ...
    for i in range(20):
        if random.randint(0, 5) < 3:#前进
            for j in range(10):
                ...
                #canvas.move(canvas_arctest,2,2)
                canvas_arctest.move(canvas)
                time.sleep(0.05)
                if Start==False:
                    break
        if Start==False:
